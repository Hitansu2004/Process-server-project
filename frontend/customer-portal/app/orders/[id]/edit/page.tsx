...
import { Search, Star, Phone, MapPin, Award, Plus, ChevronDown, Check, User, X, Mail, Save } from 'lucide-react'
...
interface Dropoff {
    id?: string // Added for tracking existing dropoffs
...
// Modal Component (Same as NewOrder)
...
export default function EditOrder() {
    ...
    const filterAndSortContacts = (contacts: ContactEntry[]) => {
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortFilter) {
                case 'highest-rated': return detailsB.currentRating - detailsA.currentRating
                case 'highest-success': return detailsB.successRate - detailsA.successRate
                case 'most-orders': return detailsB.totalOrdersAssigned - detailsA.totalOrdersAssigned
                case 'most-worked': return detailsB.successfulDeliveries - detailsA.successfulDeliveries
                default: return 0
            }
        })
        return filtered
        ...
        const fetchData = async () => {
            try {
                ...
                if (token && userId) {
                    // 1. Check Editability
                    try {
                        const editability = await api.checkOrderEditability(params.id as string, token)
                        if (!editability.canEdit) {
                            alert(`Order cannot be edited: ${editability.lockReason}`)
                            router.push(`/orders/${params.id}`)
                            return
                        }
                    } catch (e) {
                        console.error("Failed to check editability", e)
                    }
                    ...
                    setFormData({
                        ...
                        otherDocumentType: orderData.otherDocumentType || '',
                    ...
                    if (orderData.dropoffs) {
                        setDropoffs(orderData.dropoffs.map((d: any) => ({
                            id: d.id,
                            recipientName: d.recipientName,
                            dropoffAddress: d.dropoffAddress,
                            dropoffZipCode: d.dropoffZipCode,
                            dropoffType: d.dropoffType,
                            assignedProcessServerId: d.assignedProcessServerId,
                            finalAgreedPrice: d.finalAgreedPrice,
                            dropoffStateId: d.dropoffStateId,
                            dropoffCityId: d.dropoffCityId,
                            rushService: d.rushService,
                            remoteLocation: d.remoteLocation,
                            serviceType: d.serviceType || 'PROCESS_SERVICE'
                        })))
                    }

                    // 3. Load Contacts (Same as NewOrder)
                        ...
                        contacts.map(async (contact: ContactEntry) => {
                            try {
                                if (contact.activationStatus === 'NOT_ACTIVATED') {
                                    return { ...contact, processServerDetails: null }
                                }
                                const details = await api.getProcessServerDetails(contact.processServerId, token)
                                return { ...contact, processServerDetails: details }
                            } catch (error) {
                                return { ...contact, processServerDetails: null }
                            }
                        })
    ...
    const removeDropoff = (index: number) => {
        const dropoffToRemove = dropoffs[index]
        if (dropoffToRemove.id) {
            setDeletedDropoffIds([...deletedDropoffIds, dropoffToRemove.id])
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Prepare Dropoff Updates
            const dropoffUpdates = [
                // 1. Removed Dropoffs
                ...deletedDropoffIds.map(id => ({
                    dropoffId: id,
                    isToBeRemoved: true
                })),
                // 2. Existing and New Dropoffs
                ...dropoffs.map(d => ({
                    dropoffId: d.id, // Will be null for new dropoffs
                    isNew: !d.id,
                    recipientName: d.recipientName,
                    dropoffAddress: d.dropoffAddress,
                    dropoffZipCode: d.dropoffZipCode,
            ...
            const updateData = {
                ...
                dropoffUpdates: dropoffUpdates
            }

            await api.updateOrder(params.id as string, updateData, token!, user.userId)
            ...
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-gray-400 mt-1">Make changes to your order</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="card space-y-6">
                    ...
                    {/* Modification Reason */}
                    <div className="glass rounded-xl p-6 border border-yellow-200 shadow-sm bg-yellow-50/50">
                        <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                            <span className="text-2xl">📝</span> Modification Reason
                        </h3>
                        <label className="block text-sm font-medium mb-2 text-gray-700">
                            Why are you modifying this order? *
                        </label>
                        <textarea
                            value={formData.modificationReason}
                            onChange={(e) => setFormData({ ...formData, modificationReason: e.target.value })}
                            className="w-full px-4 py-3 rounded-lg glass focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none"
                            rows={3}
                            placeholder="Please explain the changes..."
                            required
                        />
                    </div>
                    ...
                    <div className="flex gap-4">
                        <button
                            type="button"
                            onClick={() => router.back()}
                            className="flex-1 px-6 py-3 rounded-lg glass hover:bg-white/10 transition-colors font-semibold"
                            disabled={saving}
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={saving || !formData.modificationReason.trim()}
